...
interface FavoritesTimelineChartProps {
  favorites: UserFavorite[];
}

export const FavoritesTimelineChart: React.FC<FavoritesTimelineChartProps> = ({ favorites }) => {

  const timelineData = useMemo(() => {
    if (favorites.length === 0) return [];

    const monthlyData = favorites.reduce((acc, favorite) => {
      ...
      return (
        <div className="bg-white dark:bg-dark-600 p-3 rounded-lg shadow-lg border border-gray-200 dark:border-dark-300">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {data.count} {data.count === 1 ? 'favorito adicionado' : 'favoritos adicionados'}
          </p>
        </div>
      );
    }
